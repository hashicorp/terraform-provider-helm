...
* `keyed_lists` - Store Kubernetes list-map fields in the rendered `manifest` AND the per-resource `resources` map as objects keyed by their identifying field instead of as arrays. Requires `manifest`. Terraform diffs an array by position, so a chart that inserts a single new entry near the front of a container's `env` makes every later entry read as changed and the plan looks as though the whole pod spec were being rewritten. Keying those lists by `name` (or `mountPath`/`devicePath` for volume mounts/devices, or `ip` for host aliases) keeps the diff to the entries that actually changed. Fields are keyed by the same field Kubernetes itself declares as the merge key wherever that is a single field; `ports` is a documented exception (keyed by `name` for usability, even though Kubernetes' real merge key is a `containerPort`/`port` plus `protocol` pair) and `sysctls` is deliberately never keyed, since Kubernetes replaces that list wholesale on any change rather than merging it. Genuinely ordered lists such as `args` and `command` are left alone, as is any list whose entries do not all carry a unique identifier. This changes the shape of both attributes, so expect a one-time diff the first time it is enabled.
