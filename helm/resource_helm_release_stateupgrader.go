	...
	return map[int64]resource.StateUpgrader{
		0: {
			StateUpgrader: func(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
				...
				// Unmarshal the old raw state as old type
				...
				// Build new metadata
				// Use null for computed fields we don't have values for
				// Terraform will populate them on the next read/apply
				newMetadata := map[string]tftypes.Value{
					"name":           metadata["name"],
					...
					"first_deployed": tftypes.NewValue(tftypes.Number, nil),
					"last_deployed":  tftypes.NewValue(tftypes.Number, nil),
				...
				var newPostRenderValue tftypes.Value
				if prObj != nil {
					newPostRenderValue = tftypes.NewValue(
					...
					newPostRenderValue = tftypes.NewValue(
						newType.AttributeTypes["postrender"],
						nil, // literal nil creates null value
					)
				}

				// Use nil so it doesn't show a spurious diff
				// Using [] or a value would cause a diff like "[] -> null" or "1 -> null"
				var newSetWoValue = tftypes.NewValue(
					newType.AttributeTypes["set_wo"],
					nil,
				)
				var newSetWoRevisionValue = tftypes.NewValue(
					tftypes.Number,
					nil,
				)
				var newSetValue tftypes.Value
				var setList []tftypes.Value
				if err := oldState["set"].As(&setList); err == nil && len(setList) == 0 {
					newSetValue = tftypes.NewValue(newType.AttributeTypes["set"], nil)
				} else {
					newSetValue = oldState["set"]
				}
				var newSetListValue = tftypes.NewValue(
					newType.AttributeTypes["set_list"],
					nil,
				)
				var newSetSensitiveValue tftypes.Value
				var setSensitiveList []tftypes.Value
				if err := oldState["set_sensitive"].As(&setSensitiveList); err == nil && len(setSensitiveList) == 0 {
					newSetSensitiveValue = tftypes.NewValue(newType.AttributeTypes["set_sensitive"], nil)
				} else {
					newSetSensitiveValue = oldState["set_sensitive"]
				}

				// pass_credentials did not exist in v0, set new default
				var newPassCredentials = tftypes.NewValue(
					tftypes.Bool,
					defaultAttributes["pass_credentials"],
				)

				newValue := tftypes.NewValue(newType, map[string]tftypes.Value{
					"metadata": tftypes.NewValue(
						newType.AttributeTypes["metadata"],
						newMetadata,
					),
					"postrender":                 newPostRenderValue,
					"set_wo":                     newSetWoValue,
					"set_wo_revision":            newSetWoRevisionValue,
					...
					"pass_credentials":           newPassCredentials,
				...
				// Provide a message to the user, informing there state has been migrated to the current framework strucuture
				resp.Diagnostics.AddWarning("UpgradeState Triggered", "Successfully migrated state from schema version 0 to current schema version")
